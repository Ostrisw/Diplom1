...
namespace Dip
{
    public partial class Glavnayaform : Form
    {
        ...
        {
            int rows = dgvSpisokZakupok.Rows.Count;
            for (int m = 0; m < rows; m++)
            {
                if (dgvSpisokZakupok.Rows[m].Cells[11].Value.ToString() == cmbKafedra.Text && dgvSpisokZakupok.Rows[m].Cells[10].Value.ToString() == cmbKalendarniyGod.Text)
                { 
                ...
                else { MessageBox.Show("В списке должна быть одна кафедра и один год закупки!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error); break; }
            }
        }
